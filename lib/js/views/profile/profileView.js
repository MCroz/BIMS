    ...
    onClickSubmit: function(evt) {
        ...
        if (updatePass) {
            if (curPassword.trim() == "" || password.trim() == "" || cpassword.trim() == "") {
                swal("Error", "Please Fill-Out All the Password Fields.", "error");
                return false;
            }
            ...
            success: function(model,response) {
                ...
                if (response.status == 1) {
                    swal("Success", response.message, "success").then((value) => {
                        ...
                        let toAddData = window.bims.mainView.changingModel.get('toUpdate');
                        toAddData+=1;
                        window.bims.mainView.changingModel.set({ toUpdate: toAddData });
